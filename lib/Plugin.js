...
class Plugin {
  ...
  ImportDeclaration(options) {
    ...
    const specifier = specifiers.find(({
      type
    }) => type === DEFAULT_SPECIFIER_FLAG);
    if (!specifier) return;
    const local = specifier.local.name;

    if (extra === REACT_FLAG) {
      if (!this.lazyDetected) {
  ...
  replaceToLazy(options, local, extra) {
    const t = this.t;
    const importCallback = t.arrowFunctionExpression([], t.CallExpression(t.Identifier('import'), [t.StringLiteral(extra)]));
    options.replaceWith(t.variableDeclaration('const', [t.variableDeclarator(t.Identifier(local), t.CallExpression(t.Identifier('lazy'), [importCallback]))]));
  }
  ...
  addLazyFlag(node) {
    ...
    if (this.reactImportDeclarationOptions) {
      ...
      importDeclarationNode.specifiers = [...importDeclarationNode.specifiers, t.ImportSpecifier(t.Identifier('lazy'), t.Identifier('lazy'))];
      this.reactImportDeclarationOptions.replaceWith(importDeclarationNode);
      return;
    }

    node.body.unshift(t.ImportDeclaration([t.ImportSpecifier(t.Identifier('lazy'), t.Identifier('lazy'))], t.StringLiteral('react')));
  }
